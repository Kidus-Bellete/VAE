...
def main():
    ...
    early_stop = EarlyStop(patience=5, save_name=save_name)
...
def train(args, model, teacher, epoch, train_loader, optimizer, log):
    ...
    for (data, _, _) in tqdm(train_loader):
        ...
        #output is reconstruction image , z(latent space)-mu-log_var used for the loss function
        ...
        #t_activations, _ = feature_extractor(data, torch.nn.Sequential(*list(teacher.children())[:-1]), target_layers=['2', '5', '7']) # for resnet18
        optimizer.zero_grad()
